 ...
 */
wp.customize.controlConstructor['ng_typography'] = wp.customize.Control.extend( {
    ready: function() {
        var control = this;
        ...
        value['text-transform'] = ( undefined !== control.setting._value['text-transform'] ) ? control.setting._value['text-transform'] : '';
        ...
        var renderSubControl = function( fontFamily, sub, startValue ) {
            ...
            for ( var i = 0, len = ngAllFonts.length; i < len; i++ ) {
                if ( fontFamily === ngAllFonts[ i ]['family'] ) {
                    if ( undefined !== ngAllFonts[ i ]['is_standard'] && true === ngAllFonts[ i ]['is_standard'] ) {
                        is_standard = true;
                    }
                    subList = ngAllFonts[ i ][ sub + 's' ]; // the 's' is for plural (variant/variants, subset/subsets)
                }
            }
            ...
            if ( 'variant' == sub ) {
                if ( 1 === subList.length || 0 === subList.length ) {
                    control.container.find( '.ng-variant-wrapper' ).css( 'display', 'none' );
                } else {
                    control.container.find( '.ng-variant-wrapper' ).css( 'display', 'block' );
                }
            }
        ...
        var picker = this.container.find ( '.ng-color-control' );
        ...
        this.container.on( 'change', '.text-align select', function() {
            // add the value to the array and set the setting's value
